...
class DefaultController extends Controller
{
    ...
    {   //create an object to store our data after the form submission
        ...
        if(  $form->isValid()) {
            ...
            $evenement= $em->getRepository('JournaleBundle:Journale')->findBy(array(
                'user' => $this->getUser()->getId()));
            ...
            $dql = $this
                ...
                ->createQuery( "SELECT e FROM  EvenementBundle:Evenement e join JournaleBundle:Journale j
                WHERE e.user ='.$id.'AND e.typeEvents.type = 'competition' or e.typeEvents.type ='rando' Group BY e.typeEvents.type ")
            ;
            $user = $this->container->get('security.token_storage')->getToken()->getUser();
            $alli= $dql->getOneOrNullResult();
            if($alli==null)


            {
                $ev=new \JournaleBundle\Entity\Evenement();
                $ev->setType('competition');
                $ev->setUser($this->container->get('security.token_storage')->getToken()->getUser());
                $journale->setEvenement($ev);
                $em->persist($ev);
    ...
    public function MAJAction(Request $request, $id)
    {
        ...
        if ($form->isSubmitted()){
            ...
            return $this->redirectToRoute("readJournal");
        }
